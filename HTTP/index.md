...
#### 问1. http和https的区别
 - http不保存状态(不做持久化处理)
  优点是 更快地处理大量事务，确保协议的可伸缩性, 解决办法：session + cookie + token
- HTTP + 通信加密(SSL) + 证书认证 + 完整性保护=HTTPS
#### 问2. HTTP(1.1)的瓶颈以及解决办法
- 一条连接上只可发送一个请求
- 请求只能从客户端开始。客户端不可以接收除响应以外的指令。
- 请求 / 响应首部未经压缩就发送。首部信息越多延迟越大，发送冗长的首部。每次互相发送相同的首部造成的浪费较多。
对应解决办法
- SPDY的设计，基于http，在 TCP/IP 的应用层与运输层之间通过新加会话层的形式运作
  1. 特点：多路复用、赋予请求优先级、压缩 HTTP 首部、推送功能
- WebSocket全双工通信
  1. 推送功能、减少通信量(首部信息少、一次链接，长期保持)
- HTTP/2.0
  1. 二进制分帧、压缩头部、多路复用、服务器推送、请求优先级、流量控制

#### 问3.常见web攻击
 - 跨站脚本攻击（Cross-Site Scripting，XSS）
 > 浏览器内运行非法的 HTML标签或 JavaScript 进行的一种攻击。
 - SQL 注入攻击
 - OS 命令注入攻击
...
#### 问4. TCP三次握手
第一次握手客户端发送报文, 并且发送SYN标志位, 以及初始序列号X, 发送完成后,客户端进入 SYN_SEND 状态

第二次握手服务器返回确认包ACK, 同时发送SYN包回去, ACK = x + 1 表示确定收到, Seq = y 是为了让客户端确认收到. 发送完成后, 服务端进入 SYN_RCVD 状态

第三次握手 客户端再次发送确认包ACK, ACK = y + 1, 表示确认收到了服务器的包. 客户端发送完毕后, 进入ESTABLISHED 状态, 服务器收到这个包以后, 也进入 ESTABLISHED 状态. 至此 TCP握手结束
#### 问5.什么是队头阻塞
 >队头阻塞是一种发生在TCP协议层的现象。在TCP协议中，每个数据包都有唯一的序号。但如果多个包中某个序号比较靠前的在传输过程中发生了丢包，其它序号靠后的包即使已顺利到达，也需要在接收方的缓冲区中等待，直到队头包重传成功才能将报文组装并返回给应用层。这个现象称为队头阻塞。
 优化办法： 优先级调度 流量控制 多路径传输


 #### 问6.浏览器缓存
  - 刷新(F5)
  - 硬性重新加载
  > 清除了关键位置的缓存；所有的资源，都会跳过缓存判断，发起真实的请求，从服务端拿资源。但本地的缓存资源(如disk里的缓存)并没有删除。 这种方式会在Request Header里添加Cache-Control:no-cache和Pragma: no-cache，也是浏览器自己的行为。
  - 清空缓存并硬性重新加载
  >相当于先删除缓存（如 disk磁盘 和 memory内存 里的缓存），再执行硬性重新加载。

  ##### 启发式缓存（即对于没有 Cache-Control 的响应）
